...
var _ = Describe("LiteLLMOrganization Controller", func() {
	Context("When reconciling a resource", func() {
		const resourceName = "test-organization"
		...
		BeforeEach(func() {
			...
			if err != nil && errors.IsNotFound(err) {
				resource := &litellmv1alpha1.LiteLLMOrganization{
					ObjectMeta: metav1.ObjectMeta{Name: resourceName, Namespace: "default"},
					Spec: litellmv1alpha1.LiteLLMOrganizationSpec{
		...
		It("should successfully reconcile the resource", func() {
			controllerReconciler := &LiteLLMOrganizationReconciler{
				Client:               k8sClient,
