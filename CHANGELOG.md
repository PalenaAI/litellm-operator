...
- **`WorkloadUnmanaged` status condition** on `LiteLLMInstance` — set when `workload.managed` is `false` and the CR still carries spec sections (`sso`, `scim`, `jwtAuth`, `oauth2Auth`, `rbac`, `security`, `logging`, `adminUI`, `caching`, `routerSettings`, `callbacks`, `passThroughEndpoints`, `secretManager`, or an installed license) that only ever take effect through a managed workload's ConfigMap. Names just the sections actually set, so a clean unmanaged CR stays quiet. `masterKey.secretRef` is now required when `workload.managed` is `false`, enforced by a CEL rule — `autoGenerate`'s Secret is only ever created by the managed-workload reconcile path, so it previously dangled silently. `LiteLLMGuardrail` now reports `Ready=False` (`InstanceUnmanaged`) instead of `Validated` when its `instanceRef` points at an unmanaged instance, since guardrail config renders through that same ConfigMap.
